...
type LogicalQuery = { $or: LocationQuery[]; $and?: never } | { $and: LocationQuery[]; $or?: never };
...
const createFilterMiddleware = (strapi: Core.Strapi) => {
  ...
  const modelsWithLocation = strapi.services[SERVICE_UID].getModelsWithLocation() as Meta[];
  ...
  return async (ctx, next) => {
    ...
    const modelCondition = (model) => model.tableName === _.snakeCase(collectionType);
    const collectionModel = !isComponentQuery
      ? modelsWithLocation.find((model) => modelCondition(model))
      : Array.from(strapi.db.metadata.values()).find(
          //@ts-ignore
    ...
    const componentAttrField =
      isComponentQuery && collectionModel.attributes[locationKeys[0].split('.')[0]];
    const componentModel =
      componentAttrField &&
      modelsWithLocation.find(
        (modelWithLocation) => modelWithLocation.uid === (componentAttrField as any).component
      );
    ...
    const fieldToFilter = isComponentQuery ? componentsToFilter[0] : locationKeys[0];
      ...
      const ids =
        isComponentQuery && matchedComponents && componentIdPairs
          ? matchedComponents.map(
              (comp) => componentIdPairs.find((pair) => pair.component_id === comp.id).entity_id
            )
          : (
    ...
    const dbQuery = query
      ?.map((item) => {
        ...
        const filters = logicalFieldsToFilter
          .map((field) => {
            const locationQueryParams = getLocationQueryParams(collectionModel, field, item);
            if (!!locationQueryParams) {
              const [lat, lng, range] = locationQueryParams;
              return `ST_DWithin(${_.snakeCase(
                field
              )}_geom, ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326), ${range ?? 0})`;
            } else {
              return false;
            }
          })
    ...
    const ids = (
      await db(collectionModel.tableName).select('id').whereRaw(wholeQuery.join(' '))
    ).map((item) => item.id);
